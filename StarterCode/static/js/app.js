...
submit.on("click", function() {
    ...
    function filterufoData(data1){
        ...
        if (inputDate && (inputDate = inputDate.trim()))
        {
            if (data1.datetime === inputDate)
                recFound = 1;
            else
                return null;
        }
        if (inputCity && (inputCity = inputCity.trim()))
        {
            if (data1.city === inputCity)
                recFound = 1;
            else
                return null;
        }
        if (inputState && (inputState = inputState.trim()))
        {
            if (data1.state === inputState)
                recFound = 1;
            else
                return null;
        }
        if (inputCountry && (inputCountry = inputCountry.trim()))
        {
            if (data1.country === inputCountry)
                recFound = 1;
            else
                return null;
        }
        if (inputShape && (inputShape = inputShape.trim()))
        {
            if (data1.shape === inputShape)
                recFound = 1;
            else
                return null;
        }
